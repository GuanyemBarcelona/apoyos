...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
    <head lang="es">
        ...
        <title>Primeres adhesions</title>
...
$ultimas=getApoyos('','',$pagina);
$total_paginas=ceil($ultimas['num_total']/NUM_RESULTADOS_PAGINA);
echo '<h2>'.$ultimas['num_total'].' firmas</h2>';
/*
...
echo '<tr>';
echo '<th>Nom</th>';
echo '<th>Activitat</th>';
echo '<th>Ciutat</th>';
echo '</tr>';
foreach ($ultimas as $apoyo) {
	$nombre = '****** ********';
	if ($apoyo['visible'] == 1) $nombre = $apoyo['nombre'];
	$ciudad = htmlentities($apoyo['ciudad']);
	if ($ciudad == "NC") $ciudad = '<span class="codigo_pais">'.$apoyo['pais'].'</span>';
	echo '<tr>';
	echo '<td>'.htmlentities($nombre).'</td>';
	echo '<td>'.htmlentities($apoyo['actividad']).'</td>';
	echo '<td>'.$ciudad.'</td>';
	echo '</tr>';
}
echo '</table><p></p>';
...
if ($pagina < $total_paginas) {
	echo '<div style="position:fixed; bottom:10px; right:10px;" class="boton"><a href="?pagina='.($pagina+1).'">Página siguiente</a></div>';
}
?>
		</div>
